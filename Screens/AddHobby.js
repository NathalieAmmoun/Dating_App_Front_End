...
function AddHobby({ navigation }){
    const [hobby, setHobby] = useState("");

    function addHobby() {
        addHobbyAPI().then((add_response) => {
      ...
      async function addHobbyAPI() {
        const response = await fetch("http://127.0.0.1:8000/api/auth/add-hobby", {
          method: "POST",
    ...
    return (
        <View style={styles.mainContainer}>
          ...
          <TouchableOpacity style={styles.loginBtn}>
            <Text onPress={addHobby} style={styles.loginText}>
              ADD HOBBY
            </Text>
          </TouchableOpacity>
          </View>
